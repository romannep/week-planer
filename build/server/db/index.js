...
export async function syncDb() {
    ...
    const whereNull = { userId: { [Op.is]: null } };
    const orphanCount = await Calendar.count({ where: whereNull });
    if (orphanCount > 0) {
        ...
        await Calendar.update({ userId: defaultUser.id }, { where: whereNull });
        await Context.update({ userId: defaultUser.id }, { where: whereNull });
    }
}
